  ...
  for(answer in data){
      ...
      div = `
          ...
          <ul>
              ...
              <li><button onclick="accept(${question_id}, ${answer_id});"><i class="fa fa-check"></i> Accept</button></a></li>
          </ul>
          <p>
            Answer by ${user}<br>
            On {date}
...
const accept = async (question_id, answer_id) => {
  let url = baseURL+`questions/${question_id}/answers/${answer_id}`;
  const response = await fetch(url, {
    method: 'POST',
    headers: head,
  });
